...
function CardDecision() {
    ...
    const getAllCard = () => {
        return axios.get("https://tproject.techpundits.net/api/decision")
            .then(response => {
        ...
        <>
            ...
            {selectedDecision && (
                <Modal className="custom-modal" show={show} onHide={handleClose} centered>
                    <Modal.Body>
                        <Editdecision decision={selectedDecision} onClose={handleClose} refresh={getAllCard} />
                    </Modal.Body>
                </Modal>
            )}
        </>
