...
function Editdecision({ decision, onClose, onUpdate, refresh}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append('decision_id', formData.decision_id);
      formDataToSend.append('decision_date', formData.decision_date);
      formDataToSend.append('_method',"PUT");
      const photos = photoUploaderRef.current.getImages();
      photos.forEach((photo, index) => {
        formDataToSend.append(`photos[${index}]`, photo.file);
      });
      const response = await axios.post(`https://tproject.techpundits.net/api/decision/${formData.id}`, formDataToSend, {
        headers: {
