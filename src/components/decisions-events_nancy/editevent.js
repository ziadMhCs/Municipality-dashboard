...
function Editevent({ service: event, onClose, refresh }) {
  ...
  const handleActivityTypeChange = (typeId) => {
    console.log('Changing activity type to:', typeId);
    setFormData({
      ...formData,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`https://tproject.techpundits.net/api/activity/${formData.id}`, formDataToSend, {
        headers: {
